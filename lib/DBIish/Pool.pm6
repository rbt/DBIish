...
method !start-single-connection() {
    $!starting-count ⚛+= 1;
    my $connection = $!driver.connect(|%!connection-args, dispose-callback => sub (*@args) {
        return self!dispose-connection-cb(|@args);
    });
    $!idle-count ⚛+= 1;
    $!starting-count ⚛-= 1;
...
method !dispose-connection-cb($connection --> Bool) {
    $!scrub-count ⚛+= 1;
    $!inuse-count ⚛-= 1;
